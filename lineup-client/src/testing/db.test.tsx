...
describe("db", () => {
  ...
  it("loaderQuery returns queryKey and queryFn resolves on successful fetch", async () => {
    ...
    const q = loaderQuery("/api/schedule/{}", "123");
    expect(q.queryKey).toEqual(["schedules", "/api/schedule/{}", "123"]);
  ...
  it("loaderQuery throws Response with original status when fetch returns non-ok", async () => {
    (authModule.getToken as unknown as Mock).mockResolvedValue("T");
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: false,
      status: 404,
    ...
    const q = loaderQuery("/api/schedule/{}", "x");
    await expect(q.queryFn()).rejects.toHaveProperty("status", 404);
  });

  it("loaderQuery throws 504 Response when fetch is aborted", async () => {
    (authModule.getToken as unknown as Mock).mockResolvedValue("TKN");
    globalThis.fetch = vi.fn().mockImplementation(() => {
      return Promise.reject(new DOMException("Aborted", "AbortError"));
    });
    const q = loaderQuery("/api/schedule/{}", "z");
    await expect(q.queryFn()).rejects.toHaveProperty("status", 504);
  });

  it("loaderQuery throws 503 Response when fetch throws unexpected error", async () => {
    (authModule.getToken as unknown as Mock).mockResolvedValue("TKN");
    globalThis.fetch = vi.fn().mockImplementation(() => {
      throw new Error("Network failure");
    });
    const q = loaderQuery("/api/schedule/{}", "y");
    try {
      await q.queryFn();
    } catch (err: unknown) {
